 ...
 * 改变函数 this 指向
 * @param {function} fn 函数
 * @param {*} thisArg 新的 this 指向
 * @returns 改变 this 后的匿名函数
 */
...
/**
 * 通过遍历添加对象 from 的属性来扩展对象 to
 * @param {Object} to 扩展对象
 * @param {Object} from 要从中复制属性的对象
 * @param {Object} thisArg 要绑定函数的对象
 * @return {Object} 对象 to 的结果值
 */
export function extend(to, from, thisArg) {
  forEach(from, (val, key) => {
    if (thisArg && typeof val === 'function') {
      to[key] = bind(val, thisArg)
    } else {
      to[key] = val
    }
  })
