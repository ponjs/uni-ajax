...
class UniAjax {
  ...
  #handleRequest(request = {}) {
    ...
    // 拦截后的请求参数
    const params = this.interceptor.request.fulfilled({ ...this.config, ..._params });

    // 拦截后的回调
    let callback = {};
    for (let item in _callback) {
      callback[item] = res => {
        _callback[item](this.#handleResponse(res));
      }
    }
  ...
  async request(params) {
    const { request, callback } = this.#handleRequest(params);
    // 如果请求拦截返回 false 终止请求
    if (!request) return;
