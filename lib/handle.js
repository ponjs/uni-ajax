...
export async function handleRequest(request) {
  let params = merge(this.config, params);
  ...
  // 拼接 url
  config.url = combineURL(this.config.baseURL, config.url);

  // 判断拼接后的 url 是否存在
  if (type(config.url) !== 'string') {
    return requestRejected.call(this, config, 'request:fail Missing required parameter `url`');
  }
