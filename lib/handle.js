...
async function handleRequest(request = {}) {
  ...
  // 合并默认参数的请求头
	let _config = clone(this.config);
	_config.header = mergeHeader(_params.method || _config.method || 'GET', _config.header);

  // 拦截后的请求参数
  let params = await this.interceptors.request.fulfilled(merge(_config, _params, { response }));

  // 清除多余的请求头
  params.header = cleanHeader(params.method, params.header);
